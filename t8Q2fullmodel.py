...
class ThreadedMac:
    def __init__(self, word_size=16, parent = None):
        ...
        self.left_queues = [Queue(maxsize=2) for _ in range(8)]  # four threads
        self.top_queues = [Queue(maxsize=2) for _ in range(8)]   # four threads
...
def simulate_systolic_array(a: torch.Tensor, b: torch.Tensor, csv_file: str = "alexnet_T82.csv"):
    # 아래 코드는 전체 매트릭스 곱을 타일 단위로 시뮬레이션하는 예시
    sa = SystolicArrayOS(arr_height=16, arr_width=16, dtype=torch.float32)
    M, K = a.shape
    K2, N = b.shape
    
    csv_file = "alexnet_T8Q2.csv"
