...
class ArbitrationUnit:
    def __init__(self, word_size=16):
        ...
        self.left_queues = [Queue(), Queue()]  # two threads
        self.top_queues = [Queue(), Queue()]   # two threads
    ...
    def arbitration_unit(self, next_pe_right=None, next_pe_bottom=None):
        chosen_thread = -1
        bypassed_threads =[]
        ...
        if not thread0_empty and not thread1_empty:
            # both threads have data
            if ((left_vals[0] != 0 and top_vals[0] != 0) and \
                (left_vals[1] != 0 and top_vals[1] != 0)) == 1:
                chosen_thread = self.current_thread
                self.current_thread = (self.current_thread + 1) % 2
            elif ((left_vals[0] != 0 and top_vals[0] != 0) and \
                not(left_vals[1] != 0 and top_vals[1] != 0)) == 1:
                chosen_thread = 0
                bypassed_threads.append(1)
                self.current_thread = (self.current_thread + 1) % 2
            elif (not(left_vals[0] != 0 and top_vals[0] != 0) and \
                    (left_vals[1] != 0 and top_vals[1] != 0)) == 1:
                chosen_thread = 1
                bypassed_threads.append(0)
                self.current_thread = (self.current_thread + 1) % 2
            else:
                for i in range(2):
                    if (left_vals[i] == 0 or top_vals[i] == 0):
                        bypassed_threads.append(i)
        else:
            # only one thread has data
            if not thread0_empty:
                chosen_thread = 0
            elif not thread1_empty and (left_vals[1] != 'p' or top_vals[1] != 'p'):
                chosen_thread = 1
...
class Systolicarray:
    ...
    def evaluate_one_cycle(self, cycle=0):
        ...
        for r in range(self.ROWS):
            for c in range(self.COLS):
                for b in range(len(self.mac_array[r][c].bypassed_threads)):
        ...
        # (2) 이제 큐를 출력: 새 입력 + 이전 사이클 데이터가 들어가 있고 아직 arbitration 전
        ...
        for r in range(self.ROWS):
            for c in range(self.COLS):
                ...
                self.mac_array[r][c].arbitration_unit(right_pe, bottom_pe)
...
def simulate_systolic_array(a: torch.Tensor, b: torch.Tensor, csv_file: str = "alexnet.csv"):
