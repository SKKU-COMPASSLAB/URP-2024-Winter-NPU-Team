...
class ArbitrationUnit:
    def __init__(self, word_size=16, parent =None):
        ...
        self.left_queues = [Queue(maxsize=2), Queue()]  # two threads
        self.top_queues = [Queue(maxsize=2), Queue()]   # two threads
        self.left_full_check = [0,0]
        self.top_full_check = [0,0]
        self.pointer = 0
                ...
                if check == "right":
                    if pe_comb[r][c+1].left_queues[idx].full():
                        ...
                        if left_vals[idx] == 0 or top_vals[idx] == 0 :
                            ...
                            chosen_thread = idx 
                            pointer_temp = (idx+1) % 2 #after thread chosen, the pointer goes to next  
                elif check == "bottom":
...
class Systolicarray:
    ...
    def set_inputs(self, left_list1, top_list1, left_list2=None, top_list2=None):
        # Thread 0의 입력
        for r in range(self.ROWS):
        ...
        self.top_in_bus_thread_list = [self.top_in_bus_thread0,self.top_in_bus_thread1]
        self.top_in_queue_thread_list = [self.top_in_queue_thread0,self.top_in_queue_thread1]
        self.left_in_bus_thread_list = [self.left_in_bus_thread0,self.left_in_bus_thread1]
        self.left_in_queue_thread_list = [self.left_in_queue_thread0,self.left_in_queue_thread1]
            ...
            for r in range(self.ROWS):
                ...
                if 0 <= c < self.COLS:
                    ...
                    if r == 0 and c == 0 and self.COLS>1 and self.ROWS>1: ## 1
                        ...
                        pe.arbitration_logic(r,c,"all")
                        
                        for i in range(2):##retry after arbitration
                        ...
                        pe.top_full_check[i] = [0,0]
                        pe.left_full_check[i] = [0,0]
                    ...
                    elif 0 < r < self.ROWS-1 and c == self.COLS-1 and self.ROWS>1 and self.COLS>1: ##6
                        #status_print(r,c,pe.left_queues,pe.top_queues,pe.chosen_thread,pe.bypassed_threads,"before arbitration")
                        pe.arbitration_unit(r,c,"bottom")
                        #status_print(r,c,pe.left_queues,pe.top_queues,pe.chosen_thread,pe.bypassed_threads,"after arbitration")
                        if pe.chosen_thread != -1:
                    ...
                    elif 0 < r < self.ROWS-1 and c == 0: ## 4
                        for i in range(2):
                            if pe.left_queues[i].full():
                                pe.left_full_check[i]=1                             
                            if self.left_in_bus_thread_list[i][r] != 'p':    
                    ...
                    elif r == self.ROWS -1 and c == 0 and self.ROWS>1 and self.COLS>1: ## 7
                        for i in range(2):
                            if pe.left_queues[i].full():
                                pe.left_full_check[i]=1                             
                            if self.left_in_bus_thread_list[i][r] != 'p':    
        ...
        for r in range(systolic.ROWS):
            ...
            for c in range(systolic.COLS):
                ...
                if len(pe.left_queues[0].queue)==0 and len(pe.left_queues[1].queue)==0 and pe.bottom_out==0 and\
                    ...
                    all(q.empty() for q in systolic.top_in_queue_thread0) and all(q.empty() for q in systolic.top_in_queue_thread1):
...
class SystolicArrayOS:
    ...
    def flush(self, addr: int) -> int:
        ...
        sliced_mem = self.onchip_mem[addr : addr + size]
        self.onchip_mem[addr : addr + size] = as_bytes
...
def simulate_systolic_array(a: torch.Tensor, b: torch.Tensor, csv_file: str = "alexnet_T2Q2.csv"):
