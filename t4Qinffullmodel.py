...
# 1) Arbitration
###############################
class Arbitration_Unit:
    def __init__(self, word_size=16):
        self.word_size = word_size
        self.accumulator_reg = 0

        # per-thread input queues
        self.left_queues = [Queue(), Queue(), Queue(), Queue()]  # four threads
        self.top_queues = [Queue(), Queue(), Queue(), Queue()]   # four threads
        self.pointer = 0
        self.right_out = 0
        self.bottom_out = 0
    ...
    def arbitration_unit(self, next_pe_right=None, next_pe_bottom=None):
        ...
        for i in range(4): #for loop for round robin arbitration
...
class Systolicarray:
    ...
    def __init__(self, ROWS=4, COLS=4, WORD_SIZE=16):
        ...
        # 2D mac array with Arbitration_Unit
        self.mac_array = []
        for r in range(ROWS):
            row_macs = []
            for c in range(COLS):
                row_macs.append(Arbitration_Unit(word_size=WORD_SIZE))
    ...
    def set_inputs(self, left_list1=None, top_list1=None, left_list2=None, top_list2=None, left_list3=None, 
                   top_list3=None, left_list4=None, top_list4=None):
        # Thread 0의 입력
...
def simulate_systolic_array(a: torch.Tensor, b: torch.Tensor, csv_file: str = "alexnet_T4Qinf.csv"):
    # 아래 코드는 전체 매트릭스 곱을 타일 단위로 시뮬레이션하는 예시
    sa = SystolicArrayOS(arr_height=16, arr_width=16, dtype=torch.float32)
    M, K = a.shape
    K2, N = b.shape
    
    csv_file = "alexnet_T4Qinf.csv"
