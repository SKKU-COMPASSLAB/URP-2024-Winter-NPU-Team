...
class ThreadedMac:
    def __init__(self, word_size=16, parent = None):
        ...
        self.left_queues = [Queue(maxsize=4) for _ in range(8)]  # four threads
        self.top_queues = [Queue(maxsize=4) for _ in range(8)]   # four threads
        self.left_full_check = [0,0,0,0,0,0,0,0]
        self.top_full_check = [0,0,0,0,0,0,0,0]
        self.pointer = 0 #for round robin arbitration
        self.right_out = 0
        self.bottom_out = 0
    ...
    def arbitration_unit(self, r=None, c=None, check = None):
        ...
        thread_empty_list = [thread0_empty,thread1_empty,thread2_empty,thread3_empty,
                             thread4_empty,thread5_empty,thread6_empty,thread7_empty]
        pointer_temp = 0
        chosen_thread = -1
        ...
        for i in range(8): #round robin arbitration
    ...
    # ...existing code for reset, etc., if needed...
def status_print(r,c,def_left_queues,def_top_queues,def_chosen_thread,def_bypassed_threads,typing=None):
    ...
    print("Thread 4:")
    print(f"  Left Queue: {list(def_left_queues[4].queue)}")
    print(f"  Top Queue: {list(def_top_queues[4].queue)}")
    print("Thread 5:")
    print(f"  Left Queue: {list(def_left_queues[5].queue)}")
    print(f"  Top Queue: {list(def_top_queues[5].queue)}")
    print("Thread 6:")
    print(f"  Left Queue: {list(def_left_queues[6].queue)}")
    print(f"  Top Queue: {list(def_top_queues[6].queue)}")
    print("Thread 7:")
    print(f"  Left Queue: {list(def_left_queues[7].queue)}")
    print(f"  Top Queue: {list(def_top_queues[7].queue)}")
...
class Systolicarray:
    ...
    def __init__(self, ROWS=4, COLS=4, WORD_SIZE=16):
        ...
        self.left_in_queue_thread4 = [Queue() for _ in range(ROWS)]  
        self.top_in_queue_thread4 = [Queue() for _ in range(COLS)]   
        self.left_in_queue_thread5 = [Queue() for _ in range(ROWS)]  
        self.top_in_queue_thread5 = [Queue() for _ in range(COLS)]   
        self.left_in_queue_thread6 = [Queue() for _ in range(ROWS)]  
        self.top_in_queue_thread6 = [Queue() for _ in range(COLS)]   
        self.left_in_queue_thread7 = [Queue() for _ in range(ROWS)]  
        self.top_in_queue_thread7 = [Queue() for _ in range(COLS)]         
        # output buses
        self.bottom_out_bus = [0]*COLS
        self.right_out_bus  = [0]*ROWS

        # 2D mac array with ThreadedMac
        self.mac_array = []
        for r in range(ROWS):
            row_macs = []
            for c in range(COLS):
                row_macs.append(ThreadedMac(word_size=WORD_SIZE,parent=self))
    ...
    def set_inputs(self, left_list1=None, top_list1=None, left_list2=None, top_list2=None, left_list3=None, top_list3=None, left_list4=None, top_list4=None
        ...
        self.top_in_bus_thread_list = [self.top_in_bus_thread0,self.top_in_bus_thread1,self.top_in_bus_thread2,self.top_in_bus_thread3,
                                       self.top_in_bus_thread4,self.top_in_bus_thread5,self.top_in_bus_thread6,self.top_in_bus_thread7]
        self.top_in_queue_thread_list = [self.top_in_queue_thread0,self.top_in_queue_thread1,self.top_in_queue_thread2,self.top_in_queue_thread3,
                                         self.top_in_queue_thread4,self.top_in_queue_thread5,self.top_in_queue_thread6,self.top_in_queue_thread7]
        self.left_in_bus_thread_list = [self.left_in_bus_thread0,self.left_in_bus_thread1,self.left_in_bus_thread2,self.left_in_bus_thread3,
                                        self.left_in_bus_thread4,self.left_in_bus_thread5,self.left_in_bus_thread6,self.left_in_bus_thread7]
        self.left_in_queue_thread_list = [self.left_in_queue_thread0,self.left_in_queue_thread1,self.left_in_queue_thread2,self.left_in_queue_thread3,
                                          self.left_in_queue_thread4,self.left_in_queue_thread5,self.left_in_queue_thread6,self.left_in_queue_thread7]
            ...
            for r in range(self.ROWS):
                ...
                if 0 <= c < self.COLS:
                    ...
                    if r == 0 and c == 0 and self.COLS>1 and self.ROWS>1: ## 1
                        ...
                        for i in range(8):##retry after arbitration
                        ...
                        pe.top_full_check[i] = [0,0,0,0,0,0,0,0]
                        pe.left_full_check[i] = [0,0,0,0,0,0,0,0]
        ...
        for r in range(systolic.ROWS):
            ...
            for c in range(systolic.COLS):
                ...
                if (len(pe.left_queues[0].queue)==0 and len(pe.left_queues[1].queue)==0 and pe.bottom_out==0 and\
                    ...
                    all(q.empty() for q in systolic.top_in_queue_thread2) and all(q.empty() for q in systolic.top_in_queue_thread3) and \
                    all(q.empty() for q in systolic.left_in_queue_thread4) and all(q.empty() for q in systolic.left_in_queue_thread5) and \
                    all(q.empty() for q in systolic.left_in_queue_thread6) and all(q.empty() for q in systolic.left_in_queue_thread7) and \
                    all(q.empty() for q in systolic.top_in_queue_thread4) and all(q.empty() for q in systolic.top_in_queue_thread5) and \
                    all(q.empty() for q in systolic.top_in_queue_thread6) and all(q.empty() for q in systolic.top_in_queue_thread7)
                    ):
...
def simulate_systolic_array(a: torch.Tensor, b: torch.Tensor, csv_file: str = "alexnet_T8Q4.csv"):
    # 아래 코드는 전체 매트릭스 곱을 타일 단위로 시뮬레이션하는 예시
    sa = SystolicArrayOS(arr_height=16, arr_width=16, dtype=torch.float32)
    M, K = a.shape
    K2, N = b.shape
    
    csv_file = "alexnet_T8Q4.csv"
...
if __name__ == "__main__":
    ...
    selected_layer_index = 1  # 선택한 레이어의 번호 (1부터 시작)
