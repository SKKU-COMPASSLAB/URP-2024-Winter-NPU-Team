...
class SystolicArrayOS:
    ...
    def execute(self, a_addr: int, b_addr: int, seq_len: int, rowA: int, colB: int, i_nnz: int) -> int:
        ...
        delay = i_nnz*(10 + round(seq_len/8)) +1
        #print (delay)
...
def simulate_systolic_array(a_: torch.Tensor, b_: torch.Tensor, f_nnz, i_nnz, csv_file: str = "alexnet_s2ta.csv"):
        ...
        for i_nnz in range (2, 9, 2):
            for layer_name in session.data.keys():
                if ".ifm" in layer_name:  # ifm 데이터만 순회
                    ...
                    if "classifier" in layer_name:  # Fully Connected (Linear) 계층이면 Transpose 적용
                        wgt = wgt.t()
                    name = f"alexnet_s2ta_f{f_nnz}_i{i_nnz}.csv"
                    # Systolic Array 적용
